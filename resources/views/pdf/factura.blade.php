  ...
  <div class="box-body">
    ...
    <table style="width:100%;margin: 10px 0px 10px 0px;">
      <thead style="background-color:#792929;color:white">
        <tr>
          <th colspan="2" style="text-align:center;padding:4px;border-radius: 0px;FONT-SIZE: 12px;">EVENTO Y DETALLES</th>
        </tr>
      </thead>
      <tbody>
          ...
          <tr style="border-style: hidden;">
            <td style="text-align:right;">LUGAR DE RECEPCION:  </td>
          <td> {{$orden->recepcion}}</td>
          </tr>

          <tr style="border-style: hidden;">
            <td style="text-align:right;">FECHA DE EVENTO:  </td>
          <td>  {{formatofecha($orden->fecha_evento)}}</td>
          </tr>

          <tr style="border-style: hidden;">
          <td style="text-align:right;">FECHA DE MONTAJE:  </td>
          <td>  {{formatofecha($orden->desde)}}</td>
          </tr>
          <tr style="border-style: hidden;">
          <td style="text-align:right;">FECHA Y HORA DE RECOGIDA:  </td>
          <td>  {{formatofecha($orden->hasta)}}</td>
          </tr>
...
<table style="width:100%;margin: 10px 0px 10px 0px;">
  ...
  <tbody>
    ...
    {{resumen($orden->producto,$orden->cantidad,$orden->cliente,$factura->concepto_descuentos,$factura->descuentos,$factura->concepto_impuestos,$factura->impuestos)}}
  </tbody>
</table>
